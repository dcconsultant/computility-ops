...
func Build(cfg config.Config) (*gin.Engine, error) {
	serverRepo, datasetRepo, renewalRepo, contractRepo, supplierRepo, metaRepo, driver, err := buildRepos(cfg)
	if err != nil {
		return nil, err
	}
	...
	h := httpapi.Handlers{
		...
		MetaData:            handler.NewMetaHandler(metaSvc),
		ReconfigMgmt:        handler.NewReconfigHandler(reconfigMgmtSvc),
		ResourcePlanning:    handler.NewResourcePlanningHandler(resourcePlanningSvc),
...
func buildRepos(cfg config.Config) (repository.ServerRepo, repository.DatasetRepo, repository.RenewalPlanRepo, repository.ContractRepo, repository.SupplierRepo, repository.MetaRepo, string, error) {
	switch cfg.StorageDriver {
	case "memory", "":
		return mem.NewServerRepo(), mem.NewDatasetRepo(), mem.NewRenewalRepo(), mem.NewContractRepo(), mem.NewSupplierRepo(), mem.NewMetaRepo(), "memory", nil
	case "mysql":
		if cfg.MySQLDSN == "" {
			return nil, nil, nil, nil, nil, nil, "", fmt.Errorf("MYSQL_DSN is required when STORAGE_DRIVER=mysql")
		}
		return mysql.NewServerRepo(cfg.MySQLDSN), mysql.NewDatasetRepo(cfg.MySQLDSN), mysql.NewRenewalRepo(cfg.MySQLDSN), mysql.NewContractRepo(cfg.MySQLDSN), mysql.NewSupplierRepo(cfg.MySQLDSN), mysql.NewMetaRepo(cfg.MySQLDSN), "mysql", nil
	default:
		return nil, nil, nil, nil, nil, nil, "", fmt.Errorf("unsupported STORAGE_DRIVER: %s", cfg.StorageDriver)
	}
}
